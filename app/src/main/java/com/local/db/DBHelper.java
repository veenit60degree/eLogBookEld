...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String TABLE_FAILED_API_TRACK               = "tbl_failed_api_track";
    ...
    public static final String FAILED_API_TRACK_LIST              = "oDriver_failed_api_track_list";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL( "CREATE TABLE " + TABLE_FAILED_API_TRACK + "(" +
                PROJECT_ID_KEY + " INTEGER, " +  FAILED_API_TRACK_LIST + " TEXT )"
        );
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_FAILED_API_TRACK);
    ...
    /* -- Create Failed Api Track table if not exist.*/
    public void CreateFailedApiTrackTable(){
        SQLiteDatabase db = this.getReadableDatabase();
        db.execSQL("CREATE TABLE " + TABLE_FAILED_API_TRACK + "(" +
                PROJECT_ID_KEY + " INTEGER, " + FAILED_API_TRACK_LIST + " TEXT )"
        );
    ...
    /* ---------------------- Insert Failed Api Track events in table -------------------- */
    public boolean InsertFailedApiTrack(int ProjectId, JSONArray jsonArray) {

        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();

        contentValues.put(PROJECT_ID_KEY, ProjectId);
        contentValues.put(FAILED_API_TRACK_LIST, String.valueOf(jsonArray));

        db.insert(TABLE_FAILED_API_TRACK, null, contentValues);
        return true;
    }
    ...
    /* ---------------------- Update Failed Api Track event in table -------------------- */
    public boolean UpdateFailedApiTrackEvent(int ProjectId, JSONArray jsonArray) {
        ...
        db.update(TABLE_FAILED_API_TRACK, contentValues, PROJECT_ID_KEY + " = ? ", new String[] { Integer.toString(ProjectId) } );
        return true;
    }
...
//        Logger.LogDebug("@@@DriverId","HelperDriverId: " +DriverId );
    ...
    /* ---------------------- Get Failed Api Track events from table-------------------- */
    public Cursor getFailedApiTrackEvent(int ProjectId) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res =  db.rawQuery("SELECT * FROM " + TABLE_FAILED_API_TRACK + " WHERE " +
                PROJECT_ID_KEY + "=?", new String[]{Integer.toString(ProjectId)});
        return res;
    }
    ...
    /* ------ Delete Failed Api Track Table ------ */
    public void DeleteFailedApiTrackEventTable() {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.execSQL("DELETE FROM "+ TABLE_FAILED_API_TRACK);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    public void CheckAllTableExistingStatus(){
        ...
        if(!isTableExists(TABLE_FAILED_API_TRACK)) {
            CreateFailedApiTrackTable();
        }
