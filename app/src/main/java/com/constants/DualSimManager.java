...
public class DualSimManager {
    ...
    public boolean isGPRS(int slotnumber) {
        ...
        try {
            ...
            int gprsInt = Integer.valueOf(GPRS_FLAG);
    ...
    public int[] getNETWORK_OPERATOR_CODE(int slotnumber) {
        ...
        try {
            if (code != null) {
                OperatorCode[0] = Integer.valueOf(code.substring(0, 3));
                OperatorCode[1] = Integer.valueOf(code.substring(3));
            }
        } catch (Exception e) {
        }
        return OperatorCode;
...
//                        }
        ...
        public String getNetworkType(int slotnumber) {
            ...
            try {
                ...
                networkType = connInfo.getNetworkTypeName(Integer.valueOf(networkType));
            ...
            try {
                if (slot == slotNumber_1) {
                    if (telephony.getPhoneType() == TelephonyManager.PHONE_TYPE_GSM) {
                         @SuppressLint("MissingPermission") GsmCellLocation location = (GsmCellLocation) telephony.getCellLocation();
                        if (location == null) {
